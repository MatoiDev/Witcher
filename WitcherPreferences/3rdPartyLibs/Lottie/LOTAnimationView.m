...
+ (nonnull instancetype)animationWithFilePath:(nonnull NSString *)filePath withTint:(nonnull UIColor *)color {
    NSMutableString *tmp;
    ...
    if (jsonString.length > 0) {
        tmp = [NSMutableString stringWithString:jsonString];

        NSString *pattern = @"\\[[+-]?([0-9]*[.])?[0-9]+,[+-]?([0-9]*[.])?[0-9]+,[+-]?([0-9]*[.])?[0-9]+,[+-]?([0-9]*[.])?[0-9]+\\]";
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:pattern options:0 error:NULL];
        NSArray* matches = [regex matchesInString:jsonString options:0 range:NSMakeRange(0, [jsonString length])];
        
        for (NSTextCheckingResult* match in matches) {
            NSString* matchText = [jsonString substringWithRange:[match range]];
            [tmp replaceOccurrencesOfString:matchText withString:[NSString stringWithFormat:@"[%f,%f,%f,%f]", red, green, blue, alpha] options:NSLiteralSearch range:NSMakeRange(0, [tmp length])];
        }
...
- (void)setAnimationWithFilePath:(nonnull NSString *)filePath withTint:(nonnull UIColor *)color {
    ...
    if (jsonDict) {
        comp = [LOTComposition animationFromJSON:jsonDict];
    } else {
        comp = [LOTComposition animationWithFilePath:filePath];
    }
